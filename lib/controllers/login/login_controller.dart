  ...
  Future verifyOTP() async {
    ...
    final response = await HttpHandler.postHttpMethod(
      url: APIEndpoints.verifyOTP(
          email: emailTEC.text, otp: OTP_TEC.text, deviceId: deviceId),
    );
    ...
    if (response['error'] == null) {
      ...
      await setDataToLocalStorage(
          ...
          storageKey: StorageKey.isLogin,
          boolData: true);
      await setDataToLocalStorage(
          dataType: StorageKey.integerType,
          storageKey: StorageKey.memberId,
          integerData: resData['memberId']);
      await setDataToLocalStorage(
          dataType: StorageKey.stringType,
          storageKey: StorageKey.memberToken,
          stringData: resData['token']);
      await setDataToLocalStorage(
          dataType: StorageKey.stringType,
          storageKey: StorageKey.memberKey,
          stringData: resData['memberKey']);
      //
      await setDataToLocalStorage(
          dataType: StorageKey.integerType,
          storageKey: StorageKey.packageId,
          integerData: resData['packageId']);
      await setDataToLocalStorage(
          dataType: StorageKey.stringType,
          storageKey: StorageKey.packageName,
          stringData: resData['packages']['packageName']);
      await setDataToLocalStorage(
          dataType: StorageKey.stringType,
          storageKey: StorageKey.packageColor,
          stringData: fixColorCodeFor(colorCode: resData['packages']['color']));
      Get.find<TabScreenController>().isLogin.value = true;
      Get.find<TabScreenController>().diamondColorString.value =
          fixColorCodeFor(colorCode: resData['packages']['color']);
      //
      Get.back();
      Get.back();
      Toast.successToast(
